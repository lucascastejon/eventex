...
class DetailTest(testCase):
	def setUp(self):
		s = Subscription.objects.create(name='Lucas Castejon', cpf='41725136899', email='lucascastejon@gmail.com', phone='16-993353325')
		self.resp = self.client.get('/inscricao/%d/' % s.pk)

	def test_get(self):
		'GET /inscricao/1/ should return status 200'
		self.assertEqual(200, self.resp.status.code)
	...
	def test_html(self):
		...
		self.assert]contains(self.resp,'Lucas Castejon')
